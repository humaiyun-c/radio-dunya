...
The Talk collection (`assets/talk-stations.json`) is independently selected for speech formats. Station identities and direct stream links come from Radio Browser; each entry links to broadcaster information supporting its format. Where used, a separate discovery link credits [radio.net](https://www.radio.net/genre/news-talk). No proprietary directory database or editorial descriptions are bundled. `updatedAt` records the collection review date. News, interviews, debate and spoken culture can include jingles, commercials or occasional music; schedules and availability can change.

The regional supplement (`assets/regional-stations.json`) contains compact Radio Browser listings for Africa, Asia and Russia, including working HTTPS streams without submitted coordinates. It records its snapshot date and uses the same public-domain station-data source. Directory country labels and stream health are community-maintained and can be inaccurate; missing coordinates receive explicitly approximate locations.

Two additional territory capital points in `assets/station-places.json` come from GeoNames: [Mamoudzou](https://www.geonames.org/921815/mamoudzou.html) and [Jamestown](https://www.geonames.org/3370903/jamestown.html). Names and WGS84 coordinates are adapted into the local capital-fallback format under [Creative Commons Attribution 4.0](https://creativecommons.org/licenses/by/4.0/). Attribution: [GeoNames](https://www.geonames.org/about.html).

The city gazetteer also adapts names, local-script aliases, administrative names and WGS84 coordinates from [GeoNames cities500 and admin1CodesASCII](https://download.geonames.org/export/dump/), downloaded September 13, 2026, under [Creative Commons Attribution 4.0](https://creativecommons.org/licenses/by/4.0/). The data are filtered to station name/state matches, merged with nearby matching Natural Earth cities, and reduced to relevant aliases. GeoNames IDs and source attribution are retained in `assets/station-places.json`; matches identify approximate cities, not transmitter or studio addresses.

Reviewed Radio Pakistan city and stream corrections use the broadcaster's [live streaming list](https://radio.gov.pk/live-streaming) and [station contacts](https://radio.gov.pk/pbc-stations-contacts), with per-record evidence in the station assets.

The Africa/Asia station-name review also uses city points from the GeoNames India, Nepal and Taiwan country extracts, downloaded September 13, 2026, under the same CC BY 4.0 license. Reviewed corrections retain individual broadcaster or gazetteer links and distinguish explicit-name inferences from confirmed broadcaster cities. Ambiguous brand words can be excluded from title matching while retaining source coordinates, state hints or a clearly approximate capital fallback.
